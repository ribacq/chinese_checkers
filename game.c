...
void link_accessible_from(Content** b, const int side, int* nb, Hex* cells, int mode){
	...
	for(i=0; i<nb_neighs; i++){
		if(get_ct(b, side, neighs[i]) != EMPTY){
			...
			if(get_ct(b, side, tmp_h) == EMPTY){
				link(side, h, neighs[i], mode);
				link(side, neighs[i], tmp_h, mode);
				in_cells = 0;
				for(j=0; j<*nb; j++)
				...
				if(!in_cells){
					...
					link_accessible_from(b, side, nb, cells, mode);
				}
			}
